...
describe("MOM", function() {
	...
	describe("Core operations", function() {
		...
		it('should create a correct UPDATE message payload', function() {
			...
			const expectedData = Buffer.concat([Buffer.from([1]), originalMessageMultiHash, updatedMessageMultiHash]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };
		...
		it('should create a correct REPLY message payload', function() {
			...
			const expectedData = Buffer.concat([Buffer.from([2]), originalMessageMultiHash, replyMessageMultiHash]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };
		...
		it('should create a correct DELETE message payload', function() {
			...
			const expectedData = Buffer.concat([Buffer.from([3]), messageMultiHash]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };
	...
	describe("Extended operations", function() {
		it('should create a correct ADD & REFER message payload', function() {
			const messageHash = "QmbHQieckNGj2KwBhpzkGSLDgezGnArL6eeuvb87YLX665";
			const messageMultiHash = multihashes.fromB58String(messageHash);
			const referredAddress = "0xF22E75d6d61D33aE38f8360574A8b72816176D20";

			const expectedData = Buffer.concat([Buffer.from([4]), messageMultiHash, Buffer.from(referredAddress)]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };

			mom.createAddAndReferTransaction(expectedAddress, messageMultiHash, referredAddress).should.jsonEqual(expectedPayload);
			expect(() => mom.createAddAndReferTransaction(expectedAddress, Buffer.from("test"), referredAddress)).to.throw();
			expect(() => mom.createAddAndReferTransaction(expectedAddress, messageMultiHash, "test")).to.throw();
		});

		it('should create a correct UPDATE & REFER message payload', function() {
			const originalMessageHash = "QmbHQieckNGj2KwBhpzkGSLDgezGnArL6eeuvb87YLX665";
			const originalMessageMultiHash = multihashes.fromB58String(originalMessageHash);
			const updatedMessageHash = "QmQpHt3EoEppWrVt2tAV22VFKTZ9NrMwfAWSsGcBZv6WKw";
			const updatedMessageMultiHash = multihashes.fromB58String(updatedMessageHash);
			...
			const expectedData = Buffer.concat([Buffer.from([5]), originalMessageMultiHash, updatedMessageMultiHash, Buffer.from(referredAddress)]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };

			mom.createUpdateAndReferTransaction(expectedAddress, originalMessageMultiHash, updatedMessageMultiHash, referredAddress).should.jsonEqual(expectedPayload);
			expect(() => mom.createUpdateAndReferTransaction(expectedAddress, Buffer.from("test"), updatedMessageMultiHash, referredAddress)).to.throw();
			expect(() => mom.createUpdateAndReferTransaction(expectedAddress, originalMessageMultiHash, Buffer.from("test"), referredAddress)).to.throw();
			expect(() => mom.createUpdateAndReferTransaction(expectedAddress, originalMessageMultiHash, updatedMessageMultiHash, Buffer.from("test"))).to.throw();
		});
		...
		it('should create a correct REMOVE ENDORSEMENT message payload', function() {
			const messageHash = "QmbHQieckNGj2KwBhpzkGSLDgezGnArL6eeuvb87YLX665";
			const messageMultiHash = multihashes.fromB58String(messageHash);

			const expectedData = Buffer.concat([Buffer.from([7]), messageMultiHash]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };

			mom.createRemoveEndorsementTransaction(expectedAddress, messageMultiHash).should.jsonEqual(expectedPayload);
			expect(() => mom.createRemoveEndorsementTransaction(expectedAddress, Buffer.from("test"))).to.throw();
		});
		...
		it('should create a correct DISAPPROVE message payload', function() {
			...
			const expectedData = Buffer.concat([Buffer.from([8]), messageMultiHash]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };
		...
		it('should create a correct REMOVE DISAPPROVAL message payload', function() {
			const messageHash = "QmbHQieckNGj2KwBhpzkGSLDgezGnArL6eeuvb87YLX665";
			const messageMultiHash = multihashes.fromB58String(messageHash);

			const expectedData = Buffer.concat([Buffer.from([9]), messageMultiHash]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };

			mom.createRemoveDisapprovalTransaction(expectedAddress, messageMultiHash).should.jsonEqual(expectedPayload);
			expect(() => mom.createRemoveDisapprovalTransaction(expectedAddress, Buffer.from("test"))).to.throw();
		});
		...
		it('should create a correct ENDORSE & REPLY message payload', function() {
			...
			const expectedData = Buffer.concat([Buffer.from([10]), originalMessageMultiHash, replyMessageMultiHash]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };
		...
		it('should create a correct DISAPPROVE & REPLY message payload', function() {
			...
			const expectedData = Buffer.concat([Buffer.from([11]), originalMessageMultiHash, replyMessageMultiHash]);
			const expectedPayload = { to: expectedAddress, value: 0, data: expectedData };
